...
def argumentParser():
    ...
    parser.add_argument('--google-sheet',
                        metavar=('<CREDENTIALS_PATH>', '<TOKEN_PATH>', '<SHEET_ID>',
                                 '<TAB_NAME>'),
                        nargs=4,
                        help='This argument takes cretentials, sheet id, tab name an token to write logs on google sheet.',
                        type=str,                                
                        required=False)
...
def logOnGoogleSpreadsheet():
    """Google Spreadsheet logger"""
    # If modifying these scopes, delete the file token.json.
    print("*************************logOnGoogleSpreadsheet*****************************************")
    ...
    CREDENTIALS_PATH = Path(__file__).parent / ARGS.google_sheet[0]
    TOKEN_PATH = Path(__file__).parent / ARGS.google_sheet[1]
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            try:
                ...
                prRed(f'{e}\nError thrown when trying to refresh expired token.')
                return
        else:
            if os.path.exists(CREDENTIALS_PATH):
                ...
                prRed(
                    f'The Google Sheets credential file `{CREDENTIALS_PATH}` does not exist or is not in the proper path. Cannot write to Google Sheets. Please refer to the README section `Google Sheets API Instructions (Optional)` for further instruction.')
                return
    ...
    try:
        ...
        sheet.values().update(
            spreadsheetId=str(ARGS.google_sheet[2]),
            range=f"'{ARGS.google_sheet[3]}'!A1",
            valueInputOption='USER_ENTERED',
            body={"values": col_names},
        ).execute()

        last_logs = dict({CURRENT_ACCOUNT: LOGS[CURRENT_ACCOUNT]})
        for index, value in enumerate(last_logs.items(), 1):
            redeem_message = ''
            new_points = None
            total_points = None
            redeem_title = None
            ...
            if int(f"{result['updates']['updatedRows']}") >= 1:
                prGreen("Row added to Google SpreadSheet succesfully")
            else:
                prRed(
                    f"Boo! Row NOT added to Google SpreadSheet, response is: {result}")
